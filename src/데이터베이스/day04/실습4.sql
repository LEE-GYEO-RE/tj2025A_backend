...
select title from books where genre = '컴퓨터' and price < 20000 and stock >= 20; 	-- 없는거임?
...
select title , author from books where author not like '이%' ; -- not like : ~가 안들어간 조건 
...
select title , pub_date from books order by pub_date desc limit 3 offset 3;  -- offset 3 : 앞의 3개 건너뜀 , limit 3 : 그 다음 3개 사용
