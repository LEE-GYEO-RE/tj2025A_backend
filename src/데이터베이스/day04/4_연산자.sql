...
# [1] as 별칭 , 조회 결과에서 속성명 변경(단 원본의 속성명과 상관없음.) / 복잡한 조회에서 속성명 단순화.
select mid from member;
select mid as 회원아이디 from member;
select mid as 회원아이디 , mname 회원명 from member;		-- as 생략하고 띄어쓰기로 구분 가능.
select mid 회원아이디 from member as m;					-- member 테이블명을 'm'으로 별칭 가능.
select mid 회원아이디 from memeber m;					-- as는 생략 가능 (띄어쓰기로 구분)

# [2] distinct , 속성값이 중복인 레코드 제거
select maddr as 주소 from member;					-- 조회 결과 속성 값들의 중복이 존재
select distinct maddr as 주소 from member;		-- 조회 결과 속성 값들의 중복 제거
...
# [4] 비교 연산자 : > 초과 , < 미만 , >= 이상 , <= 이하 , = 같다 (대입이 아님) , != 같지않다.
select * from member where mname = '블랙핑크';		-- member테이블의 mname속성의 값이 '블랙핑크'와 같은 레코드 조회
select * from member where mnumber = 4;			-- member테이블의 mnumber속성의 값이 4인 레코드 조회
select * from member where mname != '블랙핑크';	-- member테이블의 mname속성의 값이 '블랙핑크'가 아닌 레코드 조회
select * from member where not mname = '블랙핑크'; -- member테이블의 mname속성의 값이 '블랙핑크'와 같지 않은 레코드 조회
select * from member where mheight <= 162;		-- member테이블의 mheight속성의 값이 162 이하인 레코드 조회

# 논리연산자 : not 부정 , and 이면서 , or 이거나
# 기타연산자 : 속성명 between 시작값 and 끝값; , 속성명 in (값1 , 값2 , 값3 )
select * from member where mheight >= 165 and mheight <= 170; 	-- mheight속성의 값이 165 이상 이면서 170 이하인 레코드 조회
select * from member where mheight between 165 and 170;			-- mheight속성의 값이 165 이상 이면서 170 이하인 레코드 조회
select * from member where maddr = '경기' or maddr = '전남' or maddr = '경남';  -- 조건1 or 조건2 or 조건3 인 레코드 조회
select * from member where maddr in( '경기' ,'전남' , '경남' );		-- 속성명 in (값1 , 값2 , 값3 ) 인 레코드 조회

# null연산자 : 속성명 is null , 속성명 is not null
select * from member where mphone1 =  ' ';		-- 속성값이 비어있는 표현 , ' ' 빈칸 대신에 null 사용한다.
select * from member where mphone1 = null ;		-- 속성값이 null 인 레코드 조회 [x 불가능]
select * from member where mphone1 is null;		-- 속성값이 null 인 레코드 조회 [o 가능]
...
select * from member order by mdebut;		-- asc/desc 생략시 자동으로 asc 기본값 설정
select * from member order by mdebut desc;

# 주의할점 : 2개 이상의 정렬은 , (쉼표)구분 하되 , 1차 정렬에서 중복값이 있을 경우 중복값끼리 2차 정렬
# (1차) 먼저 주소(maddr) 내림차순으로 정렬 후 
# (2차) 주소가 동일한 데이터끼리 mdebut(날짜) 오름차순 정렬
select * from member order by maddr desc , mdebut asc;  	-- 주소 정렬하고 주소 같은 애들끼리 데뷔날짜 정렬 된 레코드 조회

# [6] limit 레코드수 , 조회결과 제한 , *페이징처리시 사용
# limit 시작레코드번호(0~) , 개수 
...
select * from member order by mheight desc limit 3; -- mheight 속성을 내림차순으로 정렬 후 상위 3개 레코드 조회 
