...
public class 실습13 {
    public static void main(String[] args) {
        ...
        if(object instanceof Flyable ){
            Flyable obj = (Flyable) object;
            obj.fly();
        }
        if(object instanceof Swimmable){
            Swimmable obj1 = (Swimmable) object;
            obj1.swimmable();
        }
...
//2. 이 인터페이스 안에,
// public static final로 최대 볼륨 MAX_VOLUME = 10과 최소 볼륨 MIN_VOLUME = 0 상수를 정의하세요.
...
//2. Runnable을 구현하여 각각
// "사람이 달립니다.", "자동차가 달립니다."를 출력하는 Person 클래스와 Car 클래스를 만드세요.
...
// 해당 객체의 attack() 메소드를 호출하는 Character 클래스와 useWeapon(Attackable weapon) 메소드를 만드세요.
...
//2. DataAccessObject를 구현하여 각각
// "Oracle DB에 저장", "MySQL DB에 저장"을 출력하는 OracleDao, MySqlDao 클래스를 만드세요.
...
// Greeting 인터페이스 타입의 변수를 선언하면서 익명 구현 객체를 생성하여 welcome() 메소드를 즉석에서 구현하고 호출하세요.
